...
import com.example.mihu.databinding.ActivityRegisterWorkerBinding
...
class RegisterWorkerActivity : AppCompatActivity() {
    private val viewModel: RegisterWorkerViewModel by viewModels { ViewModelFactory.getInstance(this) }
    ...
    private fun playAnimation() {
        ...
        val title = ObjectAnimator.ofFloat(binding.tvWorkerRegister, View.ALPHA, 1f)
            .setDuration(100)
        val message =
            ...
            signupButton.setOnClickListener {
                if (validateForm()) {
                    ...
                    viewModel.register(name, email, password)
                        .observe(this@RegisterWorkerActivity) { result ->
                            if (result != null) {
                                when (result) {
                                    is Result.Loading -> {
                                        showLoading(true)
                                    ...
                                    is Result.Success -> {
                                        showLoading(false)
                                        pbRegister.isVisible = false
                                        MaterialAlertDialogBuilder(this@RegisterWorkerActivity)
